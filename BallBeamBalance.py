...
cv2.createTrackbar('ki','P_I_D', 0, 100, nothing)
...
while True:
    ...
    for cnt in contours:
        ...
        if area > 100:
            ...
            centerX = x + w/2 # centerX = center point X of the object
            centerY = y + h/2 # centerY = center point Y of the object which is not used in this program
            ...
            # Get the position error for PID calculation
            ...
            try:
                ...
                errorRate = (error - previous_error) / elapsedTime
                if -30 < error < 30:
                    errorArea = errorArea + (error * elapsedTime)
                    PID_correction = kp * error + ki * errorArea + kd * errorRate
                else:
                    PID_correction = kp*error + kd*errorRate
                print('PID_correction = ',PID_correction)
                previous_error = error
                servoPos = servoNeutralPos - PID_correction
                ...
                pass
        break
    # Drawing a vertical line on the frame according to the setpoint
